  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!form.currentPassword || !form.newPassword) {
        toast.error('Mohon isi password lama dan password baru')
        setLoading(false)
        return
      }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input className="w-full h-12 px-4 bg-surface-container border-0 rounded-lg text-on-surface-variant text-body-md cursor-not-allowed opacity-70" name="name" type="text" value={form.name} readOnly />
                </div>
                <div className="space-y-2">
                  <label className="text-label-md text-on-surface-variant ml-1">Email Address</label>
                  <input className="w-full h-12 px-4 bg-surface-container border-0 rounded-lg text-on-surface-variant text-body-md cursor-not-allowed opacity-70" name="email" type="email" value={form.email} readOnly />
                </div>
              </div>
              {/* Password Section */}
              ...
              <div className="pt-6 flex flex-col md:flex-row items-center justify-end gap-6">
                <div className="flex items-center gap-2 w-full md:w-auto">
                  ...
                  <button type="submit" disabled={loading} className="px-8 py-3 bg-primary text-on-primary text-label-md rounded-lg shadow-lg hover:opacity-90 active:scale-95 transition-all w-1/2 md:w-auto disabled:opacity-50">
